Feature: Av. Save access

Background:
	Given test user login
		And I set test start date here

Scenario: Av01. Save access for current user
...
Scenario: Av02. Save access for no user
...
Scenario: Av03. Save access clear warning counting
	Given the user have being warned twice
	When I try to save the access
	Then I will receive no core error
		And and the user warning count will be 0

Scenario: Av04. Not save access if user is marked for deletion
	Given the user is marked for deletion
	When I try to save the access
	Then I will receive this core error: UserDeleted

Scenario: Av05. Not save access if user requested wipe
